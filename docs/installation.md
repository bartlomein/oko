...
Reinstall, then rerun `oko setup` in each project that uses Claude Code. Setup
now registers Oko with `"alwaysLoad": true`, so Claude Code loads the search
tool at startup instead of hiding it behind a tool search, and the managed
instructions gain one line: when an agent hands code search to a subagent, it
tells the subagent to use Oko, because subagents do not see project
instructions. Searches sent at the same time now run in parallel, up to four
at once, instead of all but one failing with "Another search is running".

### Upgrading to 0.5.0

Nothing to reconfigure: reinstall, then start a new agent session (rerun
`oko setup` per project if you want it to use the new binary right away). Every
search now makes three Jev requests at once instead of one: the shortlist,
files connected to its strongest matches, and the next keyword matches. Jev
usage per search is about three times what it was, and the wait is the slowest
of the three requests rather than their sum. What the agent is shown is
unchanged; the list of other places worth opening is better. A question that
asks for tests ("where are the tests for X") now returns tests. Each Jev
request carries an `x-oko-phase` header.
