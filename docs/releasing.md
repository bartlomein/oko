...
python3 scripts/smoke-release.py target/release-bundles/oko-v0.5.0-aarch64-apple-darwin.tar.gz
```
...
1. Commit the reviewed code and version in `Cargo.toml`/`Cargo.lock`; set the default
   ...
   Ensure CI passes.
2. Push a version tag matching Cargo exactly, for example `v0.5.0`.
