...
impl OkoServer {
    #[tool(
        name = "search",
        description = "Find code by describing its behavior when the exact name is unknown; use grep for known identifiers. Returns up to three ranked excerpts and two related definitions or callers as `path:start-end (label)` plus the exact current file contents, each line prefixed with its file line number and a tab; cite those numbers and drop the prefix when editing. Labels describe only that excerpt: `whole file` and `complete definition(s)` are shown in full; a `partial excerpt` omits surrounding code, so read the file if the rest matters. `lower confidence` marks a match rated below the relevance cutoff, shown because there was room; check it before relying on it. Results are candidates, not a complete answer: keep searching or reading when a question spans several locations. `Possible definition` is a name match only. `Other candidates` lists unshown places, best first; read them when the excerpts fall short.",
        annotations(
            read_only_hint = true,
